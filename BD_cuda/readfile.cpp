...
void write_xyz_append(int n, int n3, int n_c, int n_tail, int n_lh_g, int nc_lh, int* nc_lh_flag, int n_LH, int Nq, int Nq3, int* type, double* r, double* a, double* b, double* c, std::vector<double> core_pos, double* r_t, std::vector<double> LH_g_pos, double* r_lh, string filename){
	...
	outfile << n*4+n_c*Nq+n_tail+n_LH+nc_lh*n_lh_g << endl;
	...
	for (int i = 0; i < n_tail; i++){
		outfile << "HT" << " " << r_t[i*3] << " " << r_t[i*3+1] << " " << r_t[i*3+2] << endl;
	}

	for (int i = 0; i < n; i++){
                if (type[i]==1){
			if (nc_lh_flag[k]==1){
                        for (int j = 0; j < n_lh_g; j++){
                                z[0] = r[i*3] + a[i*3]*LH_g_pos[j*3] + b[i*3]*LH_g_pos[j*3+1] + c[i*3]*LH_g_pos[j*3+2];
                                z[1] = r[i*3+1] + a[i*3+1]*LH_g_pos[j*3] + b[i*3+1]*LH_g_pos[j*3+1] + c[i*3+1]*LH_g_pos[j*3+2];
                                z[2] = r[i*3+2] + a[i*3+2]*LH_g_pos[j*3] + b[i*3+2]*LH_g_pos[j*3+1] + c[i*3+2]*LH_g_pos[j*3+2];
                                outfile << "LHG" << " " << z[0] << " " << z[1] << " " << z[2] << endl;
                        }
			}
			k = k+1;
                }
	...
	for (int i = 0; i < n_LH; i++){
		outfile << "LH" << " " << r_lh[i*3] << " " << r_lh[i*3+1] << " " << r_lh[i*3+2] << endl;
	}
...
void write_restart(int n3, int n_tail3, int n_LH3, double* r, double* a, double* b, double* c, double* Er, double* r_t, double* r_lh){
	...
	ofstream restart_Er ("restart_Er.txt", std::ofstream::trunc);
	ofstream restart_tail ("restart_tail.txt", std::ofstream::trunc);
	ofstream restart_lh ("restart_lh.txt", std::ofstream::trunc);
	...
	for (int i=0; i<n3; i++){
		...
		restart_Er << std::fixed << std::setprecision(15) << Er[i] << endl;
	}

	for (int i=0; i<n_tail3; i++){
		restart_tail << std::fixed << std::setprecision(15) << r_t[i] << endl;
	}

	for (int i=0; i<n_LH3; i++){
		restart_lh << std::fixed << std::setprecision(15) << r_lh[i] << endl;
	}
...
void Read_restart_ini(int n3, double* r, double* a, double* b, double* c, double* Er){
	...
	ifstream ReadFile_Er("restart_Er.txt");

        i=0;
        while (getline (ReadFile_Er, line)){
                std::istringstream iss(line);
                std::vector<std::string> result((std::istream_iterator<std::string>(iss)),std::istream_iterator<std::string>());
                if (i<n3){
                        stringstream  consts(result[0]);
